...
public class SemaphoreThread {
    ...
    class WorkThread implements Runnable {
        ...
        public WorkThread(Bank bank, Semaphore semaphore) {
            this.bank = bank;
        ...
        public void run() {
            ...
            if (semaphore.availablePermits() > 0) {
                System.out.println("线程" + b + "启动，进入银行, 立即存钱");
            } else {
                System.out.println("线程" + b + "启动，进入银行, 排队等待");
            }
            try {
                semaphore.acquire();
                bank.save(10);
    ...
    public void useThread() {
        ...
        for (int i = 0; i < 10; i++) {
            // 执行一个线程
            es.submit(new Thread(new WorkThread(bank, semaphore)));
        }
        // 关闭线程池
        es.shutdown();
